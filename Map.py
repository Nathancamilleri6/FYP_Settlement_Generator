...
def restricted_flood_fill(array, xsize, ysize, start_node, replace_value, max_steps):
    ...
    stack = {(start_node[0], start_node[1])}
...
class Map:
        ...
        for x in xrange(box.minx, box.maxx):
            for z in xrange(box.minz, box.maxz):
                for y in xrange(box.maxy, box.miny, -1):
                    ...
                    if block != 0:
                        if block in self.wood_blocks or block in self.leaf_blocks or (
                                clear_decorations and block in self.decorative_blocks):
                            block_data = get_block_tuple(level, x, y, z)
                            if block_data is not None and block_data[0] in self.wood_blocks:
                        ...
                        elif block not in self.decorative_blocks:
    ...
    def clear_tree(self, x, y, nearest_tree_table):
        if 0 <= x < self.w and 0 <= y < self.h and self.trees[x, y] is not None:
            ...
            for x2 in xrange(max(0, x - distance), min(self.w, x + distance + 1)):
                for y2 in xrange(max(0, y - distance), min(self.h, y + distance + 1)):
                    ...
                    for y_step in range(self.altitudes[x2, y2] + 1, self.box.maxy):
                        ...
                        if (block in self.wood_blocks or block in self.leaf_blocks) and nearest_tree_table[x2, y2] == (
                        x, y):
    ...
    def setup_grid(self, roads_can_cross_corners, impassable_blocks):
        ...
        for x in xrange(0, self.w - 2):
            for y in xrange(0, self.h - 2):
                ...
                corner = False
                if self.altitudes[x1, y1] != self.altitudes[x2, y1] and self.altitudes[x1, y1] != self.altitudes[
                    x1, y2]:
                    corner = True
                elif self.altitudes[x2, y1] != self.altitudes[x1, y1] and self.altitudes[x2, y1] != self.altitudes[
                    x2, y2]:
                    corner = True
                elif self.altitudes[x1, y2] != self.altitudes[x1, y1] and self.altitudes[x1, y2] != self.altitudes[
                    x2, y2]:
                    corner = True
                elif self.altitudes[x2, y2] != self.altitudes[x2, y1] and self.altitudes[x2, y2] != self.altitudes[
                    x1, y2]:
                    corner = True
                if corner:
                    corners[x1, y1] = 1
    ...
    def update_grid_for_structure(self, width, length, structure_x, structure_y, road_distance, entry_distance, door_locations):
        ...
        # print("structure x1: ", x1)
        # print("structure x2: ", x2)
        # print("structure y1: ", y1)
        # print("structure y2: ", y2)
        # print("w: ", self.w)
        # print("h: ", self.h)
        # print("max x: ", max(0, x1 - road_distance))
        # print("min x: ", min(width, x2 + road_distance))
        # print("max y: ", max(0, y1 - road_distance))
        # print("min y: ", min(length, y2 + road_distance))
        ...
        for (door_x, door_y, direction, distance_x, distance_z) in door_locations:
            ...
            entry_path = self.get_entry_path_default(direction, door_y, door_x, entry_distance, distance_x, distance_z)
    ...
    def get_entry_path_default(self, direction, door_y, door_x, entry_distance, distance_x, distance_y):
        # print("door x: ", door_x)
    ...
    def collides(self, x1, y1, x2, y2, collider_blocks):
        for x in range(x1, x2):
            for y in range(y1, y2):
                # print("self.blocks: ", self.blocks[x, y])
                # print("collider blocks: ", collider_blocks)
                if self.blocks[x, y] != 2 and self.blocks[x, y] != 0 and self.blocks[x, y] not in collider_blocks:
                    print("Not in collider blocks: ", self.blocks[x, y])
    ...
    def clear_surrounding_hills(self, x1, y1, x2, y2, area_margin, altitude, max_size):
        area_x1, area_y1, area_x2, area_y2 = max(0, x1 - area_margin), max(0, y1 - area_margin), min(self.w,
                                                                                                     x2 + area_margin), min(
            self.h, y2 + area_margin)
        ...
        for x in range(area_x1, area_x2):
            for y in range(area_y1, y1):
                if altitude < self.altitudes[x, y] <= altitude + max_difference:
                    hill_points.add((x, y))
        ...
        while hill_points:
            x, y = hill_points.pop()
            if altitude < altitudes_copy[x, y] <= altitude + max_difference:
    ...
    def place_fences(self, blocks_updated, reserved_materials, impassable_blocks, materials, fence_conversion,
                     wood_source):
        ...
        for x in xrange(1, map.w - 1):
            for y in xrange(1, map.h - 1):
                if blocks_updated[x, y] not in reserved_materials and not blocks_updated[x, y] in impassable_blocks:
                    ...
                    near_road = False
                    road_altitude = -999
                    for position_d in [(-2, 0, 'e'), (0, -2, 'w'), (0, 2, 'n'), (2, 0, 's'), (-2, -2, 's'),
                                       (-2, 2, 'e'), (2, 2, 'n'), (2, -2, 'w')]:
                        if 0 < x + position_d[0] < map.w and y + position_d[1] > 0 and y + \
                                position_d[1] < map.h and blocks_updated[x + position_d[0], y + position_d[1]] == \
                                materials['road'][0]:
                            near_road = True
                    ...
                    if near_road and road_altitude <= map.altitudes[x, y]:
                        ...
                        for x2 in xrange(max(0, x - 1), min(map.w, x + 2)):
                            for y2 in xrange(max(0, y - 1), min(map.h, y + 2)):
                                if blocks_updated[x2, y2] == materials['road'][0] or blocks_updated[
                                    x2, y2] in reserved_materials:
                                    clear_space = False
                                    break
                            if not clear_space:
                                break
                        ...
                        if clear_space:
                            ...
                            for i in xrange(10):
                                ...
                                if x + x_d <= 0 or x + x_d >= map.w or y + y_d <= 0 or y + y_d >= map.h:
                                    ...
                                    faces_away = True
                                    break
                                elif map.blocks[x + x_d, y + y_d] in reserved_materials or map.blocks[
                                    x + x_d, y + y_d] == materials['road'][0]:
                                    faces_away = False
                                    break
